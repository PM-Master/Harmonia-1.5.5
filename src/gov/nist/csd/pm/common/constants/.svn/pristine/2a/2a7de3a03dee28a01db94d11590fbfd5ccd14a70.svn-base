...
public class MySQL_Statements {
    ...
    public static final String GET_RULE_ACTIONS = "select action_id from ob_action where rule_id = ?";
	...
	public static final String GET_DENY_SIMPLE_INFO = "select deny.deny_name, deny_type.name, node.name, deny.user_attribute_id, deny.is_intersection from deny join deny_type on deny.deny_type_id = deny_type.deny_type_id join node on node.node_id = deny.user_attribute_id where deny.deny_id = ?";
	...
	public static final String GET_RECORD 			= "SELECT OBJECT_DETAIL.OBJECT_NODE_ID FROM OBJECT_DETAIL WHERE OBJECT_NODE_ID IN (SELECT START_NODE_ID FROM ASSIGNMENT WHERE END_NODE_ID = ? AND DEPTH = 1) AND TEMPLATE_ID IS NOT NULL";
	public static final String GET_OBJECT_INFO 		= "select object_detail.object_node_id, object_detail.original_node_id, object_detail.include_ascedants, object_class.name from object_detail join object_class on object_class.object_class_id = object_detail.object_class_id where object_node_id = ?";
